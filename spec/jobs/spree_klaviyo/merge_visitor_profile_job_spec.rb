...
describe SpreeKlaviyo::MergeVisitorProfileJob do
  subject(:perform_job) { described_class.new.perform(klaviyo_integration.id, user.id, visitor_id) }

  let(:klaviyo_integration) { create(:klaviyo_integration) }
  let(:user) { create(:user) }
  let(:visitor_id) { 'visitor-abc-123' }
  ...
  context 'when user is not found' do
    it 'raises ActiveRecord::RecordNotFound' do
      expect {
        described_class.new.perform(klaviyo_integration.id, 999_999, visitor_id)
  ...
  context 'when klaviyo_integration is not found' do
    it 'raises ActiveRecord::RecordNotFound' do
      expect {
        described_class.new.perform(999_999, user.id, visitor_id)
      }.to raise_error(ActiveRecord::RecordNotFound)
    end
  end
